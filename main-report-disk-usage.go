...
func (c *reportDiskUsage) Run(cliConnection plugin.CliConnection, args []string) {
	outputJSON := false
	quiet := false

	fs := flag.NewFlagSet("report-disk-usage", flag.ExitOnError)
	...
	switch args[0] {
	case "report-disk-usage":
		err := c.reportDiskUsage(client, os.Stdout, outputJSON)
		if err != nil {
			log.Fatal(err)
		}
	}
...
func (c *reportDiskUsage) reportDiskUsage(client *simpleClient, out io.Writer, outputJSON bool) error {
			...
			return client.List(space.Entity.AppsURL, func(app *resource) error {
				...
				for instanceIdx, instanceStat := range stats {
					allInfo = append(allInfo, &appUsageInfo{
						...
						DiskUsage: instanceStat.Stats.Usage.Disk,
						DiskQuota: instanceStat.Stats.DiskQuota,
					})
				}
				return nil
			})
	...
	for _, info := range allInfo {
		...
		for i := range bits {
			...
			totalQuota[key], totalUsage[key] = totalQuota[key]+info.DiskQuota, totalUsage[key]+info.DiskUsage
		}
	}
	for k, quota := range totalQuota {
		allInfo = append(allInfo, &appUsageInfo{
			Key:       k,
			DiskUsage: totalUsage[k],
			DiskQuota: quota,
		})
	}
...
func (b byTotalDisk) Less(i, j int) bool {
	return (b[i].DiskQuota) < (b[j].DiskQuota)
}
...
func (c *reportDiskUsage) GetMetadata() plugin.PluginMetadata {
